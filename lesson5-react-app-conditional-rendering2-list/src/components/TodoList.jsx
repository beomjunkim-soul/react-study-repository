...
function TodoList() {
  ...
  const addTodo = () => {
    if(newTodo.trim()){
        ...
        setTodos([...todos,todo]); //기존 배열에 새항목 추가한 새배열 생성해 할당
        setNewTodo('') // 입력창 초기화
    }   
  }
  //To do 항목 삭제 함수
  ...
  const deleteTodo = (id) => {
    ...
    //to do list 중 해당 id 를 가진 to do 요소를 삭제하고 to do list를 리렌더링하기 위해서는 react hook state를 이용해야 함 => 변경을 위해서는 useState hook 함수가 반환한 두번째 요소인 set계열 함수를 이용해 업데이트해야 한다
    // filter 함수는 todo.id != id 이 true 이면 새 배열 요소로 추가 
    // 삭제할 to do id이면 != 에 의해 false 가 나올 것이고 이는 새 배열 요소에서 제외됨 -> 삭제 효과 
  ...
  return (
    <div className="todo-list">
      ...
      <div className="add-todo">  
        ...
        value={newTodo} // 입력 폼 요소 값 value 를 리액트 state 상태 값으로 관리
        // 입력 요소 value 가 change 변경될 때 react state hook 의 함수로 상태를 변경 -> 앱에 리렌더링 
        onChange={(e) => setNewTodo(e.target.value) } 

        // 아래는 key 를 눌렀을 때 발생하는 이벤트
        //keyDown 이벤트 발생시 실행될 화살표 함수 arrow function 를 등록 binding
        //addTodo 가 아니라 addTodo() 로 명시한 이유는 엔터키 이벤트 발생시에 바로 호출 즉 실행되어 todo를 추가하기 위해
        onKeyDown={(e)=>e.key==='Enter' && addTodo()} 
        />
        {/* 아래는 버튼이 클릭되어지면 (클릭 이벤트) 실행될 함수를 biding 등록 */}
      ...
      <ul className="todo-items">
        {todos.map((todo) => (
          ...
          <li key={todo.id} className={`todo-item ${todo.completed ? `completed`:undefined}`}>
            ...
            {/* 삭제 버튼이 클릭되면(클릭이벤트발생시) 실행될 화살표함수를등록
                이후 삭제 버튼을 클릭하면 함수 구현부에서 deleteTodo 함수를 실행하여 to do item 을 삭제 
            */}
      ...
      {/* to do list 가 비어있을 때 아래를 보이도록 조건 처리한다 */}
